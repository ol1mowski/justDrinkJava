...
export const useRanking = (): UseRankingReturn => {
  ...
  const updateUserScore = useCallback(async (pointsToAdd: number): Promise<UserRankingDto | null> => {
    if (!user?.id) {
    ...
    try {
      ...
      console.log(`Dodano ${pointsToAdd} punktów. Nowy wynik: ${newTotalScore}`)
      return updatedRanking
    } catch (err: any) {
      const errorMessage = err.message || 'Nie udało się zaktualizować rankingu'
  ...
  const getUserRanking = useCallback(async (userId?: number): Promise<void> => {
    ...
    try {
      ...
      const errorMessage = err.message || 'Nie udało się pobrać rankingu użytkownika'
      setError(errorMessage)
  ...
  const getTopRankings = useCallback(async (limit: number = 10): Promise<void> => {
    setLoading(true)
    setError(null)
