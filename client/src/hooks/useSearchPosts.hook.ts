...
import type { PostData, SearchPostsRequest, SearchPostsResponse } from '../utils/api'
...
export const useSearchPosts = (): UseSearchPostsReturn => {
  ...
  const performSearch = useCallback(async (query: string, offset = 0, append = false) => {
    if (!query.trim()) {
      setState(prev => ({
        ...prev,
        posts: [],
    ...
    try {
      ...
      const response: SearchPostsResponse = await apiService.searchPosts(request)
      ...
      setState(prev => ({
        ...prev,
        isLoading: false,
        error: error instanceof Error ? error.message : 'Błąd podczas wyszukiwania postów'
      }))
    }
  }, [])
  ...
  }, [state.isLoading, state.hasMore, state.posts.length, currentQuery, performSearch])
