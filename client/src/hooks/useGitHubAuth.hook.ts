...
export const useGitHubAuth = ({ onSuccess, onError, clientId }: UseGitHubAuthOptions) => {
  const initiateGitHubAuth = useCallback(() => {
    const githubClientId = clientId || import.meta.env.VITE_GITHUB_CLIENT_ID

    if (!githubClientId) {
      onError('GitHub Client ID nie jest skonfigurowane. Dodaj VITE_GITHUB_CLIENT_ID do pliku .env')
      return
    }
  ...
  const handleGitHubCallback = useCallback(async (code: string, state: string) => {
    try {
      ...
      onError(error instanceof Error ? error.message : 'Błąd podczas logowania przez GitHub')
    }
  }, [onSuccess, onError])
