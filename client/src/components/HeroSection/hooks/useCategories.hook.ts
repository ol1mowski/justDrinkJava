...
const categoryToTag = (category: CategoryDto, index: number, postCount: number = 0): CategoryTag => {
...
export const useCategories = (limit: number = 10) => {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const convertedTags = sortedCategories
        .map((category, index) => categoryToTag(category, index, category.postCount))
