...
export const StatisticItem = memo<StatisticItemProps>(({ statistic }) => {
  ...
  return (
    ...
    >
      <div
        className={`absolute inset-0 ${styles.bg} rounded-xl opacity-50 group-hover:opacity-70 transition-opacity duration-300`}
      />
      <div className="relative">
        ...
        {statistic.growth && (
          <div
            className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${styles.growth}`}
          >
            ↗ {statistic.growth}
          </div>
        )}
