...
export const PostContentError = ({ error, onRetry, className = '' }: PostContentErrorProps) => {
  const getErrorMessage = (error: PostContentApiError | Error | null): string => {
      ...
      {error instanceof PostContentApiError && (
        <details className="mt-4 text-xs text-gray-500">
          ...
          <div className="mt-2 p-2 bg-gray-100 rounded text-left">
            ...
            <p><strong>Błąd:</strong> {error.error}</p>
            <p><strong>Ścieżka:</strong> {error.path}</p>
            <p><strong>Czas:</strong> {new Date(error.timestamp).toLocaleString('pl-PL')}</p>
          </div>
        </details>
      )}
    </motion.div>
