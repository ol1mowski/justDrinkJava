...
describe('usePost', () => {
  ...
  it('should fetch post by ID successfully', async () => {
    mockApiService.getPostById.mockResolvedValue(mockPost);
  ...
  it('should handle API error', async () => {
    ...
    mockApiService.getPostById.mockRejectedValue(error);
  ...
  it('should not fetch when postId is 0', () => {
    ...
    expect(result.current.isLoading).toBe(false);
    expect(result.current.post).toBeUndefined();
    expect(mockApiService.getPostById).not.toHaveBeenCalled();
  });
  ...
  it('should handle 404 error without retry', async () => {
    ...
    expect(mockApiService.getPostById).toHaveBeenCalledTimes(1);
  });
});
