...
export const useAuthWithGitHub = (): UseAuthWithGitHubReturn => {
  ...
  const handleGitHubLogin = useCallback(() => {
    ...
    if (!githubClientId) {
      setError('GitHub Client ID nie jest skonfigurowane. Dodaj VITE_GITHUB_CLIENT_ID do pliku .env')
      return
    }
  ...
  const handleGitHubCallback = useCallback(async (code: string, state: string) => {
    ...
    try {
      ...
      console.log('🔗 Wywołuję backend API:', { code: code.substring(0, 10) + '...', state })
  ...
  return {
    ...
    clearError
  } as UseAuthWithGitHubReturn & { handleGitHubCallback: (code: string, state: string) => Promise<void> }
} 