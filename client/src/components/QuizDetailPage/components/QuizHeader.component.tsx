...
import { ClockIcon, TagIcon, FireIcon, FlagIcon } from '@heroicons/react/24/outline'
...
export const QuizHeader = memo<QuizHeaderProps>(({
  ...
  const getDifficultyIcon = (diff: string) => {
    switch (diff) {
      case 'easy': return <FireIcon className="w-4 h-4" />
      case 'medium': return <FireIcon className="w-4 h-4" />
      case 'hard': return <FireIcon className="w-4 h-4" />
      default: return <FireIcon className="w-4 h-4" />
    }
  }
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1">
          <div className="flex items-center space-x-3 mb-3">
            <span className={`inline-flex items-center space-x-1 px-3 py-1 rounded-full text-xs font-medium ${getDifficultyColor(difficulty)}`}>
